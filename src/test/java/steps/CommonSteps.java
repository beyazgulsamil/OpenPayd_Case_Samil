...
public class CommonSteps extends PageObjectStepUtilities<ObjectRepository> {
    ...
    public CommonSteps() {
        super(ObjectRepository.class, Hooks.initialiseAppiumDriver, Hooks.initialiseBrowser);
        if (Hooks.initialiseAppiumDriver)
    ...
    @Given("^(?:Click|Tap) all listed buttons from (\\w+) list on the (\\w+)$")
    public void clickListedButtons(String listName, String pageName) {
        List<WebElement> elements = pageObjectReflections.getElementsFromPage(listName, pageName);
    ...
    public void executeGenericMobileCommand(String command, String parameter, String elementName, String pageName) {
        ...
        mobileInteractions.scrollUntilFound(elementName);
        WebElement element = mobileInteractions.getElementByText(elementName, pageName);
    ...
    @Given("Send {} AndroidKey on {} element of the {}")
    public void pressAndroidKey(AndroidKey key, String elementName, String pageName) {
        PickleibAppiumDriver.get().executeScript("mobile:performEditorAction", ImmutableMap.of("action", "Search"));
    }
